function applyFilters() {
  const selectedRooms = Array.from(document.querySelectorAll('.room-filter-btn.active'))
      .map(btn => btn.dataset.room)
      .filter(room => room !== 'all');
  
  const showAllRooms = document.querySelector('.room-filter-btn[data-room="all"]').classList.contains('active');
  const showOptionalItems = document.getElementById('showOptionalItems').checked;
  const itemsList = document.getElementById('itemsList');
  
  Array.from(itemsList.children).forEach(listItem => {
      const roomMatch = showAllRooms || selectedRooms.includes(listItem.dataset.roomType);
      const optionalMatch = showOptionalItems || listItem.dataset.isOptional === 'false';
...
function setupRoomFilters() {
  ...
  ROOM_TYPES.forEach(room => {
      ...
      button.addEventListener('click', (e) => {
          e.stopPropagation();
          const allBtn = document.querySelector('.room-filter-btn[data-room="all"]');
          if (e.target === allBtn) return;
          
          button.classList.toggle('active');
          
          const anyActive = Array.from(document.querySelectorAll('.room-filter-btn'))
...
function sortItemsByPrice() {
  ...
  const filteredOutItems = items.filter(item => item.classList.contains('filtered-out'));
  ...
  itemsList.innerHTML = '';
  [...visibleItems, ...filteredOutItems].forEach(item => itemsList.appendChild(item));
}
