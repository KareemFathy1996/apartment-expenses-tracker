function loadFromStorage() {
  ...
  savedItems.forEach(item => {
      ...
      if (item.isHidden !== undefined) {
          ...
          if (hideBtn) hideBtn.textContent = item.isHidden ? 'إظهار في المجموع' : 'إخفاء من المجموع';
      }
  });
...
function saveToStorage() {
  ...
  const dataToSave = {
      items: Array.from(itemsList.children).map(listItem => ({
          ...
          minCost: listItem.dataset.minCost !== 'null' ? parseFloat(listItem.dataset.minCost) : null,
          maxCost: listItem.dataset.maxCost !== 'null' ? parseFloat(listItem.dataset.maxCost) : null,
          avgCost: listItem.dataset.avgCost !== 'null' ? parseFloat(listItem.dataset.avgCost) : null,
